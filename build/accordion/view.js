...
class Accordion {
  constructor(containerEl) {
    ...
    this.openMultiple = containerEl.getAttribute('data-open-multiple') === 'true';
    this.items = [];
    const itemEls = containerEl.querySelectorAll('.wp-block-pulsar-accordion__item');
    itemEls.forEach((itemEl, index) => {
      const accordionItem = new AccordionItem(itemEl, this);
      this.items.push(accordionItem);
      if (index === 0 && containerEl.getAttribute('data-start-open') === 'true') {
...
document.addEventListener('DOMContentLoaded', () => {
  const accordions = document.querySelectorAll('.wp-block-pulsar-accordion');
