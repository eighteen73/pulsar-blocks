/******/ (() => { // webpackBootstrap
/*!*****************************************!*\
  !*** ./src/navigation-megamenu/view.js ***!
  \*****************************************/
function calculateDistance(item) {
  const link = item.querySelector('.wp-block-navigation-item__content');
  const megamenu = item.querySelector('.wp-block-pulsar-navigation-megamenu__container');
  ...
  const existingMargin = parseFloat(window.getComputedStyle(megamenu).marginTop || 0);
...
function adjustMegamenuPosition() {
  const megamenus = document.querySelectorAll('.wp-block-pulsar-navigation-megamenu');
  if (!megamenus.length) {
    return;
  }
